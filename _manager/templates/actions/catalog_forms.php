                     ...
                     <div class="list2 fix">     </div>               
                       ...
                       <div class="list2 fix" style="display:none;">
                        ...
                        <div class="list fix" style="display:none;"> 
                            <div class="name">Fields: <span class="star">*</span></div>					
                            <select name="duration" id="duration" class="inp-small" style="width:210px;">
                                <option value='421' <?php if ($subaction=='edit') { echo ($duration == '421') ? 'selected' : ''; } ?> >Education</option>
                                <option value='422' <?php if ($subaction=='edit') { echo ($duration == '422') ? 'selected' : ''; } ?> >Health and social security</option>
                                <option value='423' <?php if ($subaction=='edit') { echo ($duration == '423') ? 'selected' : ''; } ?> >Political preferences</option>
                                <option value='424' <?php if ($subaction=='edit') { echo ($duration == '424') ? 'selected' : ''; } ?> >Tourism and migration</option>
                                <option value='425' <?php if ($subaction=='edit') { echo ($duration == '425') ? 'selected' : ''; } ?> >Agriculture</option>
                                <option value='426' <?php if ($subaction=='edit') { echo ($duration == '426') ? 'selected' : ''; } ?> >Economic</option>
                                <option value='427' <?php if ($subaction=='edit') { echo ($duration == '427') ? 'selected' : ''; } ?> >Infrastructure</option>
                                <option value='428' <?php if ($subaction=='edit') { echo ($duration == '428') ? 'selected' : ''; } ?> >Law</option>
                                <option value='429' <?php if ($subaction=='edit') { echo ($duration == '429') ? 'selected' : ''; } ?> >Religion</option>
                                <option value='430' <?php if ($subaction=='edit') { echo ($duration == '430') ? 'selected' : ''; } ?> >Mass media and communications</option>	
                                <option value='431' <?php if ($subaction=='edit') { echo ($duration == '431') ? 'selected' : ''; } ?> >Human rights and security</option>
                                <option value='432' <?php if ($subaction=='edit') { echo ($duration == '432') ? 'selected' : ''; } ?> >Youth issues</option>
                                <option value='433' <?php if ($subaction=='edit') { echo ($duration == '433') ? 'selected' : ''; } ?> >Local self-governance</option>
                                <option value='434' <?php if ($subaction=='edit') { echo ($duration == '434') ? 'selected' : ''; } ?> >Supporting CSOs</option>
                            </select>
                        </div>                        
                        <div class="list fix">
                             <div class="name">Research projects: <span class="star">*</span></div>					
                        ...
                        <div class="list2 fix">
                            <div class="icon"><a href="#"><img src="_manager/img/minus.png" width="16" height="16" alt="" /></a></div>								
                            <div class="title">Fields: <span class="star">*</span></div>								
                        </div>	
        
                        <div class="list fix">				
                        <?php
						if(l()=='ge'){
						$selPages = db_fetch_all("SELECT * FROM pages WHERE menuid = 91  AND visibility='true' AND language='ge' and deleted = 0");
							}
						else{
                        //select information from Pages
						$selPages = db_fetch_all("SELECT * FROM pages WHERE menuid = 91  AND visibility='true' AND language='en'");
						}$x=0;
						foreach($selPages as $rowPage)
						{//970
							$x++;
							$nameP = $rowPage['menutitle'];
							$idP = $rowPage['id'];
							$slug = $rowPage['slug'];					
					
							/*$idx = $_GET['idx'];*/									
							$danawevreba=array();
							if((isset($Categories))&&($Categories != "")) $danawevreba = explode(",", $Categories);
							echo '<div class="name" style="width:175px; float:left;">'.$nameP.' : <span class="star">*</span></div>';//105px
echo '<div style="width:50px; float:left;"><input type="checkbox" name="cat_'.$x.'" '.((in_array($idP, $danawevreba)) ? ' checked="checked" ':'') .' value="'.$idP.'"/></div>';//100px
						}
							echo '<input type="hidden" name="cat_cnt" value="'.$x.'" />';
						?>
                        </div>
                        ...
                        <div class="list fix">
                            <div class="name" style="line-height:16px;">Description: <span class="star">*</span></div>					
                        ...
                        <div class="list2 fix">
                            <div class="name" style="line-height:16px;">Content: <span class="star">*</span></div>					
